...
def validate_shacl(artifact: Artifact, shapes_artifact: Artifact, report: ConformanceReport) -> None:
    ...
    try:
        ...
        if not conforms:
            # Full implementation would parse results_graph for detailed findings
            # For now, we emit the summary text as a finding
            report.add_finding(Finding(
                rule_id="SHACL-VAL-01",
                severity=Severity.ERROR,
                message=f"SHACL validation failure: {results_text[:500]}...",
                evidence_links=[artifact.sha256, shapes_artifact.sha256]
            ))
